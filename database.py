# database.py - Database helper functions for ZoZi (PostgreSQL + SQLite compatibility)
...
USE_POSTGRESQL = os.getenv('DATABASE_URL') or os.getenv('USE_POSTGRESQL', 'false').lower() == 'true'

if USE_POSTGRESQL:
    # Import PostgreSQL modules
    try:
        from database_postgres import get_db, init_db, test_connection
        from database_postgres import create_user, get_user_by_email, verify_user_login
        from database_postgres import get_user_by_phone, update_user_profile
        from database_postgres import get_all_products, get_product_by_key, get_products_by_seller
        from database_postgres import search_products, update_product_stats
        from database_postgres import get_user_orders, get_order_items
        from database_postgres import get_admin_user, get_user_flags, log_admin_activity
        from database_postgres import get_seller_finances, get_seller_transactions, get_withdrawal_requests
        from database_postgres import get_daily_stats, get_category_stats, get_database_stats
...
if not USE_POSTGRESQL:
    # SQLite fallback - keep existing functions
    import sqlite3
    from werkzeug.security import generate_password_hash, check_password_hash
    DATABASE_NAME = 'zo-zi.db'
                ...
                if user and user['password']:
                    ...
                    if 'pbkdf2:' in user['password'] or 'scrypt:' in user['password'] or 'bcrypt' in user['password']:
                        ...
                        if user['password'] == password:
                            ...
                            cursor.execute('UPDATE users SET password = ? WHERE email = ?', (hashed_password, user['email']))
                            conn.commit()
                            return user
                return None
        except Exception as e:
